...
describe('ProofMode Access admin dispatch bootstrap', () => {
  ...
  it('separates pre-merge candidate ProofMode runtime proof from post-merge production proof', () => {
    ...
    expect(operationalAuthority.proofContextSemantics).toEqual({
      ...
      rulesetMigration: 'remove both legacy pre-merge ProofMode requirements from governance boundary; require the candidate ProofMode runtime context in Chief AI main exact-head gate (ruleset 20818149), bound to GitHub Actions integration 15368 with no bypass actors; do not require production ProofMode before merge; keep production verification post-merge/main-only',
    });
  });
});
