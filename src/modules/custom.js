...
export function initCustom(modal) {
  ...
  function render() {
    ...
    list.innerHTML = '';
    if (!custom.length) {
      list.innerHTML = '<div class="empty" style="border:none;padding:12px 0;text-align:left">No custom prompts yet.</div>';
      return;
    }
    custom.forEach((p, i) => {
      ...
      item.innerHTML = `
        <div class="row">
          ...
          <span class="badge cat" style="margin-left:auto">${p.cat || 'custom'}</span>
          <button class="mini-btn" data-del="${i}" style="margin-left:8px">Delete</button>
        </div>
        <div style="font-size:12px;color:var(--text-muted);margin-top:4px">${p.sub || ''}</div>
      `;
      item.querySelector('[data-del]').addEventListener('click', e => {
        ...
        if (nc) nc.textContent = custom.length;
      });
  ...
  document.getElementById('saveCustom')?.addEventListener('click', () => {
    ...
    const versions = {};
    platforms.forEach(p => { versions[p] = body; });
