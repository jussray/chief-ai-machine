...
1. **Observe** the exact repository, branch, environment, entry points, tests, and configuration.
2. **Orient** around product intent, project boundaries, user impact, durability, reversibility, and prior decisions.
3. **Redteam** assumptions, privacy, security, authorization, abuse, data isolation, cost, failure, and rollback.
4. **Decide** one explicit minimal path with success and stop conditions.
5. **Act** with the smallest coherent patch.
6. **Verify** using tests, logs, diffs, or inspected configuration.
7. **Report** reality, risk, decision, action, proof, and the next approval gate.
