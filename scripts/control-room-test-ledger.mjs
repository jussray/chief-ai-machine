  ...
  for (const run of Array.isArray(checkRuns) ? checkRuns : []) {
    ...
    if (
      ...
      || timestamp(run.completed_at ?? run.started_at)
        >= timestamp(current.completed_at ?? current.started_at)
    ) {
...
async function githubJson(url, token) {
  ...
  if (!response.ok) {
    throw new Error(
      `GitHub check lookup failed (${response.status}): ${(await response.text()).slice(0, 500)}`,
    );
  }
...
export async function observeExactHeadChecks(env = process.env) {
  ...
  const observerCheckName = clean(
    env.CONTROL_ROOM_LEDGER_SELF_CHECK || 'Publish exact-head test ledger',
  );
  const outputPath = clean(env.CONTROL_ROOM_TEST_LEDGER_PATH)
    || 'artifacts/control-room-test-ledger.json';
  ...
  if (!repository || !sha || !token) {
    throw new Error(
      'GITHUB_REPOSITORY, EXPECTED_HEAD_SHA/GITHUB_SHA, and GITHUB_TOKEN are required.',
    );
  }
