...
async function githubJson(path) {
  ...
  if (response.status === 403 || response.status === 429) {
    throw new ProofModeGitHubError(
      "source_rate_limited",
      "GitHub temporarily refused the public evidence request. Try again later."
    );
  }
  ...
  if (!response.ok) {
    throw new ProofModeGitHubError(
      "source_error",
      `GitHub evidence request failed with HTTP ${response.status}.`
    );
  }
...
export async function loadPublicRepositoryEvidence({ owner, repo, ref }) {
  ...
  if (!treeSha) {
    throw new ProofModeGitHubError(
      "source_error",
      "GitHub did not return a tree for the requested ref."
    );
  }
  ...
  const [tree, readme, workflowRuns, deployments] = await Promise.all([
    ...
    optionalGitHubJson(`${repoPath}/actions/runs?head_sha=${encode(headSha)}&per_page=20`, {
      workflow_runs: [],
    }),
  ...
  const deploymentEvidence = await Promise.all(
    deployments.slice(0, 5).map(async (deployment) => {
      const statuses = await optionalGitHubJson(
        `${repoPath}/deployments/${deployment.id}/statuses?per_page=1`,
        []
      );
      return {
        environment: deployment.environment || "unspecified",
        latestState: statuses[0]?.state || "unknown",
      };
    })
  );
  ...
  return {
    ...
    workflows: Array.isArray(workflowRuns.workflow_runs)
